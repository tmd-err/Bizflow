...
export default function NewAdjustmentPage() {
  ...
  const products = useMemo(() => {
    const map = new Map<number, { id: number; sku: string; name: string; minimum_stock: number }>();
    for (const s of stock) map.set(s.product_id, { id: s.product_id, sku: s.product_sku, name: s.product_name, minimum_stock: s.minimum_stock });
  ...
  async function submit() {
    ...
    try {
      ...
      showSuccess("Adjustment created.");
      window.location.href = "/dashboard/inventory/adjustments";
    } catch (e) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div>
          ...
          <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Physical count, damage…" />
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border">
        <table className="min-w-full divide-y">
          <thead className="bg-muted/40">
            <tr>
              <th className="px-4 py-3 text-left text-xs uppercase text-muted-foreground">Product</th>
              <th className="px-4 py-3 text-right text-xs uppercase text-muted-foreground">System Qty</th>
              <th className="px-4 py-3 text-right text-xs uppercase text-muted-foreground">Actual Qty</th>
              <th className="px-4 py-3 text-right text-xs uppercase text-muted-foreground">Diff</th>
              <th />
            </tr>
          </thead>
              ...
              <tr key={i}>
                <td className="px-4 py-3">
                  ...
                  >
                    ...
                    {products.map((p: { id: number; sku: string; name: string }) => (
                      <option key={p.id} value={p.id}>{p.sku} — {p.name}</option>
                    ))}
                  </select>
                </td>
                <td className="px-4 py-3 text-right text-sm font-mono">{item.system_quantity.toLocaleString()}</td>
                <td className="px-4 py-3 text-right">
                  <Input type="number" step="0.001" className="w-28 text-right"
                    value={item.actual_quantity || ""}
                    onChange={(e) => updateItem(i, { actual_quantity: parseFloat(e.target.value) || 0 })}
                  />
                </td>
                <td className={`px-4 py-3 text-right text-sm font-mono ${(item.actual_quantity - item.system_quantity) < 0 ? "text-red-600" : (item.actual_quantity - item.system_quantity) > 0 ? "text-emerald-600" : ""}`}>
                  {(item.actual_quantity - item.system_quantity).toLocaleString()}
                </td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => removeItem(i)} className="text-xs text-red-500 hover:underline">Remove</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex gap-3">
        ...
        <Button onClick={submit} disabled={saving}>{saving ? "Saving…" : "Create Adjustment"}</Button>
      </div>
    </div>
  );
}